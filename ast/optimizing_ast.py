...
DATA_DIRS = {
    "Neg": "./datasets/neg", # Directory containing negative (Noise/Background) samples (Label 0.0)
    "Drone": "./datasets/drone",
    "Piston": "./datasets/piston", 
    "Turbofan": "./datasets/turbofan",
    "Turboprop": "./datasets/turboprop",
    "Turboshaft": "./datasets/turboshaft"
}
...
BATCH_SIZE = 4 # Actual batch size passed to the DataLoader
GRADIENT_ACCUMULATION_STEPS = 4 # Steps to accumulate gradients before optimization step (Effective Batch Size = 4 * 8 = 32)
LEARNING_RATE = 5e-5 # Maximum learning rate for the OneCycleLR scheduler
NUM_EPOCHS = 20 # Total number of training passes over the dataset
...
AUGMENTATION_MULTIPLIER = 2 # How many times to duplicate each original training sample through augmentation
...
# def finetune(train_dataloader, val_dataloader, class_weights):
...
#                 # # Primary loss: Weighted Binary Cross-Entropy with Logits
#                 # bce_loss = F.binary_cross_entropy_with_logits(
#                 #     logits, 
#                 #     input_data['labels'],
#                 #     pos_weight=pos_weight # Apply class imbalance correction
#                 # )
...
#                 # # Retrieve the accumulated MoE load balancing loss
#                 # moe_aux_loss = MOE_LOSS_CACHE['loss']
#                 # # Combine the primary loss and the auxiliary loss, scaled by the coefficient
...
#                 # Calculate binary predictions (Sigmoid > 0.5)
#                 predictions = (torch.sigmoid(outputs.logits) > 0.5).long().flatten()
